...
def globals():
    ...
    folder = Path('QUENCH_HO-neg_quartic')
    ...
    M1 = np.load(f"matrix_300_HO_1.npy")
    M2 = np.load(f"matrix_300_neg_quartic.npy")


    return folder, hbar, m, ω, l1, N, L, Nx, x, delta_x, P, t, t_final, delta_t, T, M1, M2


if __name__ == "__main__":

    folder, hbar, m, ω, l1, N, L, Nx, x, delta_x, P, t, t_final, delta_t, T, M1, M2 = globals()

    HO_GS = np.zeros(N, complex)
    for n in tqdm(range(N)):
        y = IC_integrand(x, n)
        HO_GS[n] = trapezoid(y, x, delta_x)
