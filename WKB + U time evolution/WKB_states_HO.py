...
def WKB_states(x, Energies):
    ...
    for n, E in enumerate(Energies):
        ...
        wkb[x < a - δminus] = np.exp(-integral_left) / (3 * np.sqrt(Q[x < a - δminus])) # I scaled these down to 1/3 instead of 1/2
...
def plot_states(states, Energies):
    ax = plt.gca()
...
def globals():
    ...
    return hbar, m, ω, Nx, x, delta_x, ϵ0, Energies_ϵ0, N, δminus, δplus, δRight, δLeft


if __name__ == "__main__":

    hbar, m, ω, Nx, x, delta_x, ϵ0, Energies_ϵ0, N, δminus, δplus, δRight, δLeft = globals()

    print("\n#################### Harmonic oscillator ####################")
    # print(f"\n{Energies_ϵ0 = }\n")
    states_ϵ0 = WKB_states(x, Energies_ϵ0) 
    plot_states(states_ϵ0, Energies_ϵ0)

    # parity flipped states
    P_states_ϵ0 = [state[::-1] for state in states_ϵ0]
    # plot_states(P_states_ϵ0, Energies_ϵ0)

    normalised_states_ϵ0, normalised_P_states_ϵ0 = PT_normalised_states(x, states_ϵ0, P_states_ϵ0)
    # plot_states(normalised_states_ϵ0, Energies_ϵ0)
