...
def spatial_wavefunctions(N, x, evals, evects):
    ...
    # plt.plot(x, (1/2) * (x / l1**2) ** 2, alpha=0.4, color="black")
    plt.plot(x, (1/2) * (x / l2**2) ** 2, alpha=0.4, color="black")
...
matrix = Matrix(N)
# np.save(f"matrix_100_HO_1.npy", matrix)
# matrix = np.load(f"matrix_100_HO_1.npy")
np.save(f"matrix_100_HO_2.npy", matrix)
# matrix = np.load(f"matrix_100_HO_2.npy")
